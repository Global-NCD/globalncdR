chap.forestplot = function(datain, var){
  
  frm = as.formula(paste0(var, "~factor(visit) + arm"))
  
  model = lme(frm, random=~1|id, data = eval(parse(text = datain)), na.action=na.exclude, subset=visit>0&age_rand>=50)
  ...
  model = lme(frm, random=~1|id, data = eval(parse(text = datain)), na.action=na.exclude, subset=visit>0&lpg==1)
  lpg1 = c(matrix(unlist(intervals(model,which="fixed")),ncol=3) [ 3,c(2,1,3) ], summary(model)$tTable[3,5])
  
  model = lme(frm, random=~1|id, data = eval(parse(text = datain)), na.action=na.exclude, subset=visit>0&lpg==0)
  lpg0 = c(matrix(unlist(intervals(model,which="fixed")),ncol=3) [ 3,c(2,1,3) ], summary(model)$tTable[3,5])
  
  model = lme(frm, random=~1|id, data = eval(parse(text = datain)), na.action=na.exclude, subset=visit>0&sbp>120)
